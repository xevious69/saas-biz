...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content" id="overview">
      ...
      <section className="properties-section" id="properties"><div className="section-heading"><div><h2>Your properties</h2><p>A quick look at your real estate portfolio</p></div><a href="#properties">View all properties →</a></div><div className="property-grid">{portfolio.map((property) => { const net = property.monthlyIncome - property.monthlyCosts - property.monthlyMortgage; return <article className="property-card" key={property.id}><div className={`property-image property-${property.id}`}><span>{property.type}</span><button aria-label={`More options for ${property.name}`}>•••</button></div><div className="property-body"><div className="property-title"><div><h3>{property.name}</h3><p>⌖ {property.location}</p></div><strong>{compactMoney.format(property.value)}</strong></div><div className="property-stats"><div><span>Monthly income</span><strong>{money.format(property.monthlyIncome)}</strong></div><div><span>Net cash flow</span><strong className="positive-text">+{money.format(net)}</strong></div><div><span>Mortgage left</span><strong>{compactMoney.format(property.mortgageBalance)}</strong></div></div><div className="occupancy"><span>Occupancy</span><strong>{property.occupancy}%</strong><div><i style={{ width: `${property.occupancy}%` }} /></div></div></div></article>; })}</div></section>
